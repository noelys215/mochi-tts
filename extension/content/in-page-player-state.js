(() => {
  ...
  function seekTarget(currentTime, duration, deltaSeconds) {
    ...
    return Math.max(0, Math.min(safeDuration, safeCurrent + safeDelta));
  }
  ...
  function map(shared = {}) {
    ...
    return {
      ...
      remainingSeconds,
      remainingLabel: duration ? `-${formatTime(remainingSeconds)}` : null,
      primary,
      showPlayback: showTransport && hasPlayableAudio,
      showProgress: showTransport && duration > 0,
      rewindDisabled: currentTime <= 0,
      forwardDisabled: duration <= 0 || currentTime >= duration,
      showCancel: generationActive && generation.cancellable !== false,
      showRetry: mode === "failed",
      showSpinner: initialGeneration,
      showIndeterminate: initialGeneration,
      showErrorBar: mode === "failed",
      showSecondaryLoading: mode === "buffering",
    };
  }

  globalThis.__mochiAudioInPagePlayerState = Object.freeze({ formatTime, map, seekTarget });
})();
